...
describe('SourcemapStore', () => {
  it('passes through dev source files (no map needed)', () => {
    const s = new SourcemapStore({ baseDir: tmpDir });
    const r = s.resolveFrame('app', 'v1', { filename: 'http://localhost:5188/src/App.tsx', line: 10, column: 5 });
    expect(r.resolved).toBe(true);
    expect(r.source).toBe('/src/App.tsx');
  });

  it('returns unresolved when no map available for bundle', () => {
    const s = new SourcemapStore({ baseDir: tmpDir });
    const r = s.resolveFrame('app', 'v1', { filename: 'https://h/assets/index-Abcd1234.js', line: 1, column: 100 });
    expect(r.resolved).toBe(false);
  });

  it('resolves a frame via uploaded sourcemap', () => {
    // 生成一个 sourcemap：bundle 第 1 行第 0 列 -> src/foo.ts 第 42 行第 7 列
    ...
    const r = s.resolveFrame('app', 'v1', { filename: 'https://h/assets/index-Abcd1234.js', line: 1, column: 0 });
    expect(r.resolved).toBe(true);
  ...
  it('falls back to "latest" version', () => {
    ...
    const r = s.resolveFrame('app', 'v9.9', { filename: 'https://h/bundle-a1b2c3d4.js', line: 1, column: 0 });
    expect(r.resolved).toBe(true);
    expect(r.line).toBe(5);
  });

  it('resolveStack resolves a full stack string', () => {
    ...
    const frames = s.resolveStack('app', 'v1', '    at foo (https://h/assets/index-Xy9z8w7t.js:1:100)');
    expect(frames.length).toBe(1);
